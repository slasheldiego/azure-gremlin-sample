...
-->
<h1>JavaScript Gremlin Language Variant</h1><p><a href="https://tinkerpop.apache.org">Apache TinkerPop™</a> is a graph computing framework for both graph databases (OLTP) and graph analytic systems
...
console.log(names);</code></pre><h1>Sample Traversals</h1><p>The Gremlin language allows users to write highly expressive graph traversals and has a broad list of functions that 
...
<h2>Create Vertex</h2><pre class="prettyprint source lang-javascript"><code>/* if we want to assign our own ID and properties to this vertex */
...
const createVertex = async (vertexId, vlabel) => {
  ...
  return vertex.value;
};</code></pre><h2>Find Vertices</h2><pre class="prettyprint source lang-javascript"><code>/**
...
};</code></pre><h2>Update Vertex</h2><pre class="prettyprint source lang-javascript"><code>const { cardinality: { single } } = gremlin.process;
...
const updateVertex = async (vertexId, label, name) => {
  ...
  return vertex.value;
};</code></pre><p>NOTE that versions suffixed with &quot;-rc&quot; are considered release candidates (i.e. pre-alpha, alpha, beta, etc.) and 
